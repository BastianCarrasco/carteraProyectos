...
export const generarInformeProyectos = (
  proyectos,
  formatDate,
  formatCurrency
) => {
  try {
    ...
    proyectos?.forEach((proyecto, index) => {
      ...
      const datos = [
        ...
        `• Académico responsable: ${proyecto.academico || "N/A"} ${
          proyecto.jefe ? "(Jefe de proyecto)" : ""
        }`,
        ...
        `• Tipo de apoyo: ${proyecto.tipo_apoyo || "N/A"} - ${
          proyecto.detalle || "N/A"
        }`,
    ...
    yPosition = addTextWithPageCheck(
      `• Total de proyectos: ${proyectos?.length || 0}`,
      marginLeft,
      yPosition
    );
    ...
    const inversionTotal =
      proyectos?.reduce((sum, p) => sum + (p.monto || 0), 0) || 0;
    yPosition = addTextWithPageCheck(
...
export const exportarCSV = () => {
  try {
    ...
    // Crear filas CSV
    const csvRows = proyectosParaCSV
      .map((proyecto) =>
        campos
          .map((campo) => {
